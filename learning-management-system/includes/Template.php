...
class Template implements TemplateInterface {
	...
	public function get( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		if ( $filter_template !== $template ) {
			if ( ! file_exists( $filter_template ) ) {
				...
				Utils::doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'learning-management-system' ), '<code>' . $filter_template . '</code>' ), '1.0.0' );
				return;
			}
		...
		if ( ! empty( $args ) && is_array( $args ) ) {
			if ( isset( $args['action_args'] ) ) {
				Utils::doing_it_wrong(
					__FUNCTION__,
					__( 'action_args should not be overwritten when calling get_template.', 'learning-management-system' ),
					'1.0.0'
				);
	...
	public function set_cache( $cache_key, $template ) {
		...
		$cache->set( $cache_key, $template, 'learning-management-system' );

		$cached_templates = $cache->get( 'cached_templates', 'learning-management-system' );
		...
		$cache->set( 'cached_templates', $cached_templates, 'learning-management-system' );
	}
	...
	public function get_cache( $cache_key ) {
		...
		return (string) $cache->get( $cache_key, 'learning-management-system' );
	}
