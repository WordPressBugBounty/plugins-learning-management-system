...
class NotificationServiceProvider extends AbstractServiceProvider implements BootableServiceProviderInterface {
	...
	public function boot(): void {
		...
		add_action( 'masteriyo_update_user_course', array( $this, 'schedule_course_start_notification_to_student' ), 10, 2 );
		...
		add_action( 'masteriyo_rest_quiz_attempt_update_answer_points', array( $this, 'schedule_new_review_notification_to_student' ), 10, 2 );
	...
	/**
	 * Schedule quiz attempt notification to student.
	 *
	 * @since 2.8.0
	 *
	 * @param int $id User course id.
	 * @param \Masteriyo\Models\QuizAttempt $object The user course object.
	 * @param \Masteriyo\Models\QuizAttempt $quiz_attempt
	 */
	public function schedule_new_review_notification_to_student( $id, $quiz_attempt ) {

		$result = masteriyo_get_setting( 'notification.student.quiz_attempt' );
	...
	public function schedule_course_start_notification_to_student( $id, $user_course ) {
		...
		$notification = masteriyo_get_notification_by_user_post_id_and_type( $user_course->get_user_id(), $user_course->get_course_id(), 'course_enroll' );
		...
		if ( 0 !== $order_id || UserCourseStatus::ACTIVE !== $user_course->get_status() ) {
			return;
		}
	 ...
	 */
	public function schedule_order_created_notification_to_student( $order ) {
