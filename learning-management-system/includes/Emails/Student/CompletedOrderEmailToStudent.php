...
class CompletedOrderEmailToStudent extends Email {
	...
	public function trigger( $order ) {
		...
		$to_addresses_setting = masteriyo_get_setting( 'emails.student.completed_order.to_address' );
		$to_address           = array();
	...
	public function get_subject() {
		 ...
		 */
		$subject = apply_filters( $this->get_full_id() . '_subject', masteriyo_get_setting( 'emails.student.completed_order.subject' ) );
		$subject = is_string( $subject ) ? trim( $subject ) : '';
		$subject = empty( $subject ) ? masteriyo_get_default_email_contents()['student']['completed_order']['subject'] : $subject;
	...
	public function get_content() {
		$content = masteriyo_string_translation( 'emails.student.completed_order.content', 'masteriyo-email-message', masteriyo_get_setting( 'emails.student.completed_order.content' ) );
		$content = is_string( $content ) ? trim( $content ) : '';

		if ( empty( $content ) ) {
			$content = masteriyo_get_default_email_contents()['student']['completed_order']['content'];
		}
	...
	public function get_reply_to_name() {
		 ...
		 */
		$reply_to_name = apply_filters( $this->get_full_id() . 'reply_to_name', masteriyo_get_setting( 'emails.student.completed_order.reply_to_name' ) );
	...
	public function get_reply_to_address( $reply_to_address = '' ) {
		 ...
		 */
		$reply_to_address = apply_filters( $this->get_full_id() . 'reply_to_address', masteriyo_get_setting( 'emails.student.completed_order.reply_to_address' ) );
	...
	public function get_from_name() {
		 ...
		 */
		$from_name = apply_filters( $this->get_full_id() . '_from_name', masteriyo_get_setting( 'emails.student.completed_order.from_name' ) );
	...
	public function get_from_address( $from_address = '' ) {
		 ...
		 */
		$from_address = apply_filters( $this->get_full_id() . '_from_address', masteriyo_get_setting( 'emails.student.completed_order.from_address' ) );
