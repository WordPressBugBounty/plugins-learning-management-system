...
class NewQuestionReplyEmailToStudent extends Email {
		...
		if ( $reply ) {
			$placeholders = $placeholders + array(
				...
				'{reply_link}'    => wp_kses_post(
					'<a href="' . $course->start_course_url() . '" class="email-template--button">View Reply</a>'
				),
			);
		}
	...
	public function get_reply_to_name() {
		$reply_to_name = apply_filters( $this->get_full_id() . '_reply_to_name', masteriyo_get_setting( 'emails.student.new_question_reply.reply_to_name' ) );
	...
	public function get_reply_to_address( $reply_to_address = '' ) {
		$reply_to_address = apply_filters( $this->get_full_id() . '_reply_to_address', masteriyo_get_setting( 'emails.student.new_question_reply.reply_to_address' ) );
	...
	public function get_from_name() {
		$from_name = apply_filters( $this->get_full_id() . '_from_name', masteriyo_get_setting( 'emails.student.new_question_reply.from_name' ) );
	...
	public function get_from_address( $from_address = '' ) {
		$from_address = apply_filters( $this->get_full_id() . '_from_address', masteriyo_get_setting( 'emails.student.new_question_reply.from_address' ) );
