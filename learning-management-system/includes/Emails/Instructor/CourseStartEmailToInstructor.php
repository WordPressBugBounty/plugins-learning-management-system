...
class CourseStartEmailToInstructor extends Email {
	...
	public function trigger( $user_course ) {
		...
		$to_addresses_setting = masteriyo_get_setting( 'emails.instructor.course_start.to_address' );
		$to_address           = array();
	...
	public function get_subject() {
		 ...
		 */
		$subject = apply_filters( $this->get_full_id(), masteriyo_get_setting( 'emails.instructor.course_start.subject' ) );
		$subject = is_string( $subject ) ? trim( $subject ) : '';
		$subject = empty( $subject ) ? masteriyo_get_default_email_contents()['instructor']['course_start']['subject'] : $subject;
	...
	public function get_content() {
		$content = masteriyo_string_translation( 'emails.instructor.course_start.content', 'masteriyo-email-message', masteriyo_get_setting( 'emails.instructor.course_start.content' ) );
		$content = is_string( $content ) ? trim( $content ) : '';

		if ( empty( $content ) ) {
			$content = masteriyo_get_default_email_contents()['instructor']['course_start']['content'];
		}
	...
	public function get_reply_to_name() {
		 ...
		 */
		$reply_to_name = apply_filters( $this->get_full_id() . 'reply_to_name', masteriyo_get_setting( 'emails.instructor.course_start.reply_to_name' ) );
	...
	public function get_reply_to_address( $reply_to_address = '' ) {
		 ...
		 */
		$reply_to_address = apply_filters( $this->get_full_id() . 'reply_to_address', masteriyo_get_setting( 'emails.instructor.course_start.reply_to_address' ) );
	...
	public function get_from_name() {
		 ...
		 */
		$from_name = apply_filters( $this->get_full_id() . '_from_name', masteriyo_get_setting( 'emails.instructor.course_start.from_name' ) );
	...
	public function get_from_address( $from_address = '' ) {
		 ...
		 */
		$from_address = apply_filters( $this->get_full_id() . '_from_address', masteriyo_get_setting( 'emails.instructor.course_start.from_address' ) );
