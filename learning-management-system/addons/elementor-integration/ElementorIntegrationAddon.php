...
class ElementorIntegrationAddon {
	 ...
	 *
	 * The `:not(.masteriyo-course-carousel)` is load-bearing. The Course Carousel
	 * widget also emits `masteriyo-course-list-display-section` (it needs that
	 * ancestor for the layout card rules to reach its cards at all), and it renders
	 * no view-mode switcher of its own — so without the exclusion a non-default
	 * carousel on the canvas would hide a *default*-layout Course List widget's
	 * switcher, which is neither what this rule means nor what it did before.
	 */
	public function enqueue_preview_styles() {
		...
		wp_add_inline_style(
			...
			'body:has(.masteriyo-course-list-display-section[data-layout]:not(.masteriyo-course-carousel)) .masteriyo-courses-view-mode-section{display:none !important;}'
		);
	}
	...
	/**
	 * Whether the current request is the Elementor editor or its preview iframe.
	 *
	 * Both layout-context consumers below must skip in the editor, because both
	 * read the layout out of the *saved* post content and the canvas may be showing
	 * an unsaved one. The preview stylesheet handles layout-based hiding there
	 * instead.
	 *
	 * It is one method rather than the condition written out at each site because
	 * the two must agree: an earlier pass through this tree kept the guard in
	 * `add_layout_body_class()` and dropped it from
	 * `set_course_list_layout_context()`, which left the editor canvas rendering
	 * the saved layout's chrome. A single named answer makes that divergence
	 ...
	 * is hidden for non-default layouts even when rendered by a separate Toolbar widget.
	 */
	 ...
	 *
	 * Elementor renders categories in insertion order. We hook at PHP_INT_MAX
	 * so all categories are registered first, then we splice ours to the front.
	 *
	 * @param \Elementor\Elements_Manager $elements_manager
	 */
	...
	public function register_widgets( $widgets_manager ) {
		$widgets = array(
			...
			new CourseArchiveViewModeWidget(), // Hidden from panel; kept for existing pages.
	...
	protected function generate_css_for_widget_icons() {
		...
		$icons = array(
			...
			array_merge(
				array(
					'class' => 'masteriyo-course-cohort-widget-icon',
				),
				Helper::get_widget_icon_urls( 'course-cohort-widget-icon' )
			),
			array_merge(
				array(
					'class' => 'masteriyo-course-enrollment-expiration-widget-icon',
				),
				Helper::get_widget_icon_urls( 'course-enrollment-expiration-widget-icon' )
			),
			array_merge(
				array(
			...
			array_merge(
				array(
					'class' => 'course-prerequisites-widget-icon',
				),
				Helper::get_widget_icon_urls( 'course-prerequisites-widget-icon' )
			),
			array_merge(
				array(
					'class' => 'social-share-meta-widget-icon',
				),
				Helper::get_widget_icon_urls( 'social-share-meta-widget-icon' )
			),
			array_merge(
				array(
	...
	public function enqueue_elementor_editor_scripts() {
		...
		$img_base_url = masteriyo_get_plugin_url() . '/assets/img/';
		...
		wp_localize_script(
			...
			array(
				...
				'single_course_layout_images'  => array(
					'default' => $img_base_url . 'single-course-default-layout.png',
					'layout1' => $img_base_url . 'single-course-layout1-layout.png',
					'minimal' => $img_base_url . 'single-course-minimal-layout.png',
				),
				'course_archive_layout_images' => array(
					'default' => $img_base_url . 'course-default-layout.png',
					'layout1' => $img_base_url . 'course-layout1-layout.png',
					'layout2' => $img_base_url . 'course-layout2-layout.png',
				),
				...
				// Used by the "set as active template" checkbox after a new template is created.
				'rest_url'                     => rest_url(),
				...
				'i18n'                         => array(
					...
					'template_activated'             => sprintf(
						/* translators: %s: the product's name */
						__( 'Template set as active in %s settings.', 'learning-management-system' ),
						masteriyo_get_plugin_name()
					),
				),
			)
	 ...
	 *
	 * Injects the "Set as active template" checkbox into Elementor's New Template
	 * dialog when a Masteriyo template type is selected. The checkbox intent is
	 * stored in sessionStorage and consumed by the editor script on load.
	 */
	...
	public function add_use_template_for_masteriyo_action( $actions, $post ) {
		...
		$label         = esc_html(
			sprintf(
				/* translators: %s: the product's name */
				__( 'Use template for %s', 'learning-management-system' ),
				masteriyo_get_plugin_name()
			)
		);
		$url = '';
	...
	public function add_post_states( $post_states, $post ) {
		...
		if ( CourseArchivePageDocumentType::TYPE_SLUG === $template_type ) {
			if (
				...
				$post_states['masteriyo_used_template'] = sprintf(
					/* translators: %s: the product's name */
					__( 'Used by %s', 'learning-management-system' ),
					masteriyo_get_plugin_name()
				);
			}
